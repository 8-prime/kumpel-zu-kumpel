...
- Files stream sequentially in wire chunks smaller than 16 KiB. The sender reads batches of 16 chunks (255 KiB) and encrypts those chunks concurrently, then sends them in sequence. The complete read batch must fit within the 1 MiB of credit ahead of acknowledged download progress; data-channel backpressure also applies. Decryption overlaps earlier download writes, but sequence validation and processing remain ordered. The receive queue, including decrypting/decrypted frames, is capped at 2 MiB of encrypted input / 256 frames. The receiver combines payloads into at most 64 KiB download writes and returns credit only after its browser download stream accepts them. Its service-worker stream buffers at most one write. Browser/OS networking and disk buffers are managed by the browser.
...
Unit tests check that offers over 10 GiB do not read file data before acceptance, a stalled output bounds sender read-ahead, the transfer resumes without byte loss, disconnects reject pending acceptance, and flooding the encrypted receive queue fails safely. They also force encryption/decryption to finish out of order, detach download buffers, check partial final writes, and reject disk output with work queued. Same-machine browser tests can connect using host candidates and do not establish cross-network NAT compatibility. A passing 11 GiB run is not a claim that 50–100 GiB files or every browser/device have been tested.

### Throughput benchmark

`npm run benchmark:transfer` starts an isolated Vite server on `127.0.0.1:5183` and transfers a real temporary 128 MiB file between two local browser instances for three rounds. It uses the actual encrypted WebRTC transport and browser download manager, verifies SHA-256 for every download, and removes its temporary files and browser profiles. It exchanges connection descriptions inside the test harness, so no signaling or STUN server is needed.

Set `PLAYWRIGHT_CHANNEL=msedge` to use installed Edge on Windows, or install Playwright Chromium. `SENDER_CHANNEL` and `RECEIVER_CHANNEL` override each side independently. `BENCHMARK_MIB` and `BENCHMARK_ROUNDS` control file size and repetitions. JSON output includes MiB/s, file-read count, selected-route RTT, and accumulated time in file reads, crypto, download writes, and backpressure waits. Concurrent operation timings overlap and must not be added together as wall-clock time. These are same-machine measurements, not predictions of Wi-Fi or Ethernet throughput. For stock Firefox download checks, use `test:firefox-native` as described above.
