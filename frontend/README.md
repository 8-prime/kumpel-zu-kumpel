...
- Both peers show a per-file transfer speed beside active progress, sampled every half second over approximately the last two seconds. Speeds use payload bytes (B/s, KiB/s, MiB/s, etc.): the sender measures bytes passed to the data channel; the receiver measures bytes accepted by its browser download stream. The rate drops to zero during a stall, resets for each file, and disappears when the file completes or stops.
